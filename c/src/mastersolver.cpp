...
MasterSolver::MasterSolver(U32 nbColors, U32 nbPositions):MasterGame(nbColors, nbPositions),m_indexPastGuessSetTreated(0), m_pCurrentNodeCombinaison(NULL)
{
  buildScoreSet();
...
vectorCombinaison* MasterSolver::solve()
{
  ...
  m_pastGuessSet.clear();m_possibleSolutionSet.clear();m_indexPastGuessSetTreated=0;
...
bool MasterSolver::isCombiCompatible(Combinaison &givenTestedCombinaison, Combinaison &combinaisonToTest,tScore &score)
{
...
void MasterSolver::guessNextPattern(Combinaison &nextPatternCombi, bool bFastSearch)
{
  ...
  if(bFastSearch)
  {
    for(listCombinaison::iterator itor = m_possibleSolutionSet.begin(); itor != m_possibleSolutionSet.end(); ++itor)
    {
#if NUMBER_OF_THREADS <= 1
      updateFromIteration(*itor, maximumGuessScore, nextPatternCombi);
#else
	  updateFromIterationMT_emit(*itor);
#endif
    }
  }
  else
...
void MasterSolver::createThreads()
{
	...
	#ifdef __DEBUG__
	pthread_mutex_init(&MT_DEBUG_mutex, NULL);
	#endif

	/* Initialize mutex and condition variable objects */
	pthread_mutex_init(&MTcomb_mutex, NULL);
	pthread_cond_init (&MTcomb_cond, NULL);
	
	pthread_mutex_init(&MTmaximumGuessScore_mutex, NULL);
    pthread_cond_init (&MTmaximumGuessScore_cond, NULL);
...
void* MasterSolver::updateFromIterationWT(void *t)
{
	...
	while(m_MTthreadExit[id])
	{
	  ...
	  while(m_MTvectorComb.size() == 0)
	  {//if the signal is not yet sent, we need to wait the signal, in order to receive it
...
U32 MasterSolver::testCurrentPattern(const Combinaison &currentGuessPattern)
{
  U32 thisScore=~0;
  for(vectorScore::iterator itorScore = m_scoreSet.begin(); itorScore != m_scoreSet.end(); ++itorScore)
  {
    U32 tempScore=countRemovals(currentGuessPattern,*itorScore);
...
U32  MasterSolver::countRemovals(const Combinaison &guessCombinaison,tScore &score)
{
  ...
  {
    U32 blackPigs, whitePigs;
    getCorrection(guessCombinaison,*itor,blackPigs, whitePigs);
