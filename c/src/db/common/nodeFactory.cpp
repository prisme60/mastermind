...
namespace dbserver {
    ...
    bool NodeFactory::findNode(U32 parentnode_id, int nbOfElement, node_path_unit unitDataPath[], node_combi &nodeCombi_result, extra_info_struct &extra) {
        ...
        if (nbOfElement > 0) {
            while (file_stream.good()) {
                cerr << "read" << endl;
                TEST()
                file_stream.read(reinterpret_cast<char*> (&nodeCombi_result), sizeOf_node_combi);
                TEST()
                if (file_stream.gcount() == sizeOf_node_combi) {//we should never read other size than sizeOf_node_combi or 0 (eof), otherwise it means that the file is corrupted
                    ...
                    if ((nodeCombi_result.parentnode_id == parentnode_id) &&
                            ...
                            (nodeCombi_result.correction_white == unitDataPath[0].correction_white)) {// node with the id has been found
                        bResult = true;
                        ...
                        for (U32 i = 0; i < nbPosition; i++) {
                            cerr << "[" << hex << static_cast<int> (nodeCombi_result.combinaison[i]) << "]";
                        }
                        cerr << endl;
                        break;
                    }
        ...
        if (bResult) {//if the parent node has been found, we can try to search a child node!
        ...
        } else {//here the searched node, that it is not present in the database
    ...
    bool NodeFactory::readCombinaison(stringstream &sPath, U32 maxPosition, U32 &position, U8* &pCombinaisons) {
        ...
        if (static_cast<size_t> (position) != string::npos) {
            ...
            for (U32 combiItem = 0; combiItem < maxPosition; combiItem++) {
                ...
                if ((separator >= '0') && (separator <= '9')) {//we have aread a digit, we need to go back to the previous position (in order to the chaarcater to be read by the U32 variable)
    ...
    bool NodeFactory::fillNodeInit(int nbOfElement, node_path_unit unitDataPath[], U8 combinaisons[]) {
        ...
        U8 buffer[STATIC_NODE_COMBI_SIZE];
        extra_info_struct extra;
        if (!findNode(UNKNOWN_NODE_ID, nbOfElement, unitDataPath, *reinterpret_cast<node_combi*> (buffer), extra)) {//node need to be filled, because is is not present in the db
    ...
    bool NodeFactory::fillNode(U32 parentnode_id, int nbOfElement, node_path_unit unitDataPath[], U8 combinaisons[], node_combi &nodeCombi_result) {
        ...
        bool bResult = false;
        if (nbOfElement > 0) {
            file_stream.seekp(0, ios::end);
            TEST()
            S32 nbBytes = file_stream.tellp();
            TEST()
            if (nbBytes == -1)
                nbBytes = 0;
            ...
            cerr << "address unitDataPath black\t" << reinterpret_cast<long> (&unitDataPath[0].correction_black) << "\t-->\t" << static_cast<int> (unitDataPath[0].correction_black) << endl;
            cerr << "address unitDataPath white\t" << reinterpret_cast<long> (&unitDataPath[0].correction_white) << "\t-->\t" << static_cast<int> (unitDataPath[0].correction_white) << endl;
            memcpy(nodeCombi_result.combinaison, combinaisons, sizeOf_node_combi);
            TEST()
            for (U32 i = 0; i < nbPosition; i++) {
                cerr << "[" << hex << static_cast<int> (nodeCombi_result.combinaison[i]) << "]";
            }
            cerr << endl;
            file_stream.write(reinterpret_cast<char*> (&nodeCombi_result), sizeOf_node_combi);
            file_stream.flush();
            TEST()
