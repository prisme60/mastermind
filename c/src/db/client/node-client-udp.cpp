...
#ifdef TEST_UNIT_DBCLIENT
    int main(/*int argc, char* argv[], char* arge[]*/) {
        dbclient::nodeClientUdp cli("localhost", 1500);
        string result;
        string request[] = {
        ...
        for (unsigned int i = 0; i < (sizeof (request) / sizeof (string)); i++) {
            cout << request[i];
...
namespace dbclient {
    ...
    nodeClientUdp::nodeClientUdp(std::string _host, int _serverPort) : host(_host), serverPort(_serverPort) {
        ...
        if (hostInfo == NULL) {
            cerr << "Probleme dans l'interpretation des informations d'hote : " << host << "\n";
            exit(1);
        }
        ...
        if (socketDescriptor < 0) {
            cerr << "Impossible de creer le socket\n";
            exit(1);
        }
