...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        for (S32 i = 1; i < argc; i++) {
            if (strcasecmp(argv[i], STR_FULL_ENUMERATION) == 0)
                ...
                nb_Colors = strtoul(argv[i] + strlen(STR_NB_COLORS), nullptr/*no invalid character treatment*/, 10/*decimal base*/);
            } else if (strncasecmp(argv[i], STR_NB_POSITIONS, strlen(STR_NB_POSITIONS)) == 0) {
                nb_Positions = strtoul(argv[i] + strlen(STR_NB_POSITIONS), nullptr/*no invalid character treatment*/, 10/*decimal base*/);
            } else {
                choice = cCHOICE_HELP;
    ...
    switch (choice) {
        case cCHOICE_FULL_ENUMERATION: fullEnumeration(nb_Colors, nb_Positions);
            break;
        case cCHOICE_RANDOM_COMBINAISON:randomCombinaison(nb_Colors, nb_Positions);
            break;
        case cCHOICE_UNITARY_TEST: unitaryTest(nb_Colors, nb_Positions);
            break;
        default:
        case cCHOICE_HELP: help();
            break;
    }
...
static int fastcgi_main() {
    ...
    while (FCGX_Accept_r(&request) == 0) {
        ...
        char *value;
        if ((value = FCGX_GetParam("REQUEST_METHOD", request.envp)) != nullptr) {
            cout << "<p>REQUEST_METHOD=" << value << "</p>";
        }
        if ((value = FCGX_GetParam("QUERY_STRING", request.envp)) != nullptr) {
        ...
        if (combi.getNextCombinaison(nbColors) == nullptr)
